...
Generator.create({
  name: 'view',
  aliases: ['v'],
  usage: 'iron {generate, g}:{view, v} [path/]<name> [--where] [--dir]',
  description: 'Generate scaffolding for a View.',
  examples: [
    'iron g:view todos/todos_item',
    'iron g:v todos_item'
  ]
}, function (args, opts) {
  var self = this;
  ...
  if (! this.isConfigFile(root))
    this.writeFile('.iron/config.json', this.template('.iron/config.js', Config.defaults()) );
  ...
  try {
   ...
   _.each(fileExtensions, function (ext) {
     ...
     if ( realExtension ) {
      ...
      self.writeFile(path.join(root, filepath), tmplContent, opts);
     }
