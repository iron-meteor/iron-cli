...
Command.prototype = {
  ...
  packageInstallingSpinner: function(package) {
    var spinner = ['-', '\\', '|', '/'];
    var i = 0;
    var timer = setInterval(function () {
      log("Installing the package " + package + '... ' + spinner[i]);
      (i === 3) ? i = 0 : i++;
    }, 200);
    return timer;
  },

  installPackage: function(manager, package) {
    var future = new Future;
    var command = manager + ' add ' + package;
    var dir = this.findAppDir();
    var spinner = this.packageInstallingSpinner(package);
  ...
  writeDir: function (dir, mode, opts) {
    ...
    try {
      ...
      var configOpts = Config.options();
      if (! _.isEmpty(configOpts) && configOpts.addGitkeepFiles)
