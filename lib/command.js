...
/**
 * Run this command with an args array and options. Usually
 * this will be called from the executable with ARGV.
 */
...
Command.prototype.run = function run(args, opts) {
  ...
  try {
    ...
    if (e instanceof Command.UsageError) {
      ...
      self.logError("You must be in an iron project to run this command");
    } else {
      //um what?
      throw e;
