...
Command.prototype = {
  ...
  getConfigExtensionFor: function(fileType) {
    ...
    console.log("config.options: ", configOpts);
    if (configOpts !== undefined) {
      if (fileType === 'css') {

        if (configOpts.views.renderCssFiles && configOpts.views.cssFileExtensionType)
          return configOpts.views.cssFileExtensionType;
        else
          return false;
  ...
  fileExtensions: function(opts) {
    var addExts = [{fileType: 'html', dotType: '.html', realExtension:'.html' }, {fileType: 'js', dotType:'.js',realExtension:'.js'}],
        cssPckgs = ['stylus', 'less', 'scss'],
        optExts = _.pick(opts, cssPckgs.concat(['css', 'less', 'scss', 'sass', 'styl', 'stylus']));

    if (_.contains(optExts, false)) // if arg has been passed in to exclude
      return addExts;
    else if (_.contains(optExts, true)) // else if arg has been passed in to include
      return addExts.concat(this.getPassedInCssType(optExts));
    else // else just get defaults
      return (this.getConfigExtensionFor('css')) ? addExts.concat({fileType: 'css', dotType: '.css', realExtension: this.getConfigExtensionFor('css'), }) : addExts;
