...
Command.prototype = {
  ...
  // Can we make this so that it gets the real extension of
  // any file type, i.e., .coffee, .jade.
  getRealExtension: function() {
    return this.getCssOrPreprocessor().realExtension;
  },
  ...
  getCssOrPreprocessor: function() {
    if (this.hasPackage('less'))
      return {type: '.css', realExtension: '.less'};
    else if (this.hasPackage('scss'))
      return {type: '.css', realExtension: '.scss'};
    else if (this.hasPackage('stylus'))
      return {type: '.css', realExtension: '.styl'};
    else
      return {type: '.css', realExtension: '.css'};
  },


  fileExtensions: function(opts) {
    var cssExts = [],
    addExts = [{type: '.html', realExtension:'.html' }, {type: '.js', realExtension:'.js'}],
