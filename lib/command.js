...
Command.prototype = {
  ...
  //   switch (true) {
  //     case exts.css:
  //       ext.realExtension = '.css';
  //       break;
  //     case exts.less:
  //       ext.realExtension = '.less';
  //       break;
  //     case exts.scss:
  //     case exts.sass:
  //       ext.realExtension = '.scss';
  //       break;
  //     case exts.styl:
  //     case exts.stylus:
  //       ext.realExtension = '.styl';
  //       break;
  //   }
  //   return ext;
  // },

  getConfigExtensionFor: function(fileType) {
    ...
    if (configOpts !== undefined) {
      ...
      if (fileType === 'html') {
        if (configOpts.view.html.create && configOpts.view.html.extension)
          return configOpts.view.html.extension;
        else
          return false;
      };

      if (fileType === 'css') {
        if (configOpts.view.css.create && configOpts.view.css.extension)
          return configOpts.view.css.extension;
        else
          return false;
      };

      if (fileType === 'js') {
        if (configOpts.view.js.create && configOpts.view.js.extension)
          return configOpts.view.js.extension;
        else
          return false;
      }
  ...
  fileExtensions: function(opts) {
    // var addExts = [{fileType: 'html', dotType: '.html', realExtension:'.html' }];
        // cssPckgs = ['stylus', 'less', 'scss'],
        // optExts = _.pick(opts, cssPckgs.concat(['css', 'less', 'scss', 'sass', 'styl', 'stylus']));

    // if (_.contains(optExts, false)) // has been passed in to exclude?
    //   return addExts;
    // else if (_.contains(optExts, true)) // has arg been passed in to include?
    //   return addExts.concat(this.getPassedInCssType(optExts));
    // else // just get the defaults
      return [
          { fileType: 'html', dotType: '.html', realExtension: this.getConfigExtensionFor('html') },
          { fileType: 'css', dotType: '.css', realExtension: this.getConfigExtensionFor('css') },
          { fileType: 'js', dotType: '.js', realExtension: this.getConfigExtensionFor('js') }
        ];
  ...
  runSubCommand: function (command, args, opts) {
    ...
    command = parts[0];
    firstArg = parts[1];
    console.log("command: ", command); // g
    console.log("firstArg: ", firstArg); // config
