...
/**
 * If we have a srcpath of foo/bar.js and an engine for js
 * that maps to coffee, then look for a template file named
 * foo/bar.js.coffee. So the schema for template files looks like
 * this: 
 *
 * <template_name>.<type>[.<engine>]
 *
 * You should omit the engine in the template file name if it's the
 * same as the type. So instead of mytemplate.js.js you would just
 * create mytemplate.js and it's assumed that the engine is js.
 *
 * The path paramter to this function is automatically adjusted to
 * look for the template file for the configured engine. For example,
 * if your js engine is 'coffee' then a src path of '/path/to/template.js'
 * will be translated to '/path/to/template.js.coffee'.
 *
 */
module.exports.rewritePathForEngine = function rewritePathForEngine(/* path parts */) {
  var config = CurrentConfig.get();

  var srcpath = _.toArray(arguments).join(path.sep);
  var srcdir = path.dirname(srcpath);
  var basefilename = path.basename(srcpath).replace(path.extname(srcpath), '');
  var type = path.extname(srcpath).replace('.', '');


  // if we have config engines then let's rewrite the
  // srcpath extension for the appropriate engine. if srcpath
  // is a directory and doesn't have a type, then nothing to do
  // here.
  if (type && config && config.engines) {
    var engine = config.engines[type];

    // if there's no engine defined just fall back
    // to using the srcpath directly
    if (engine) {
      srcpath = srcpath + '.' + engine;
    }
  }
};
 ...
 * rewritten to use the appropriate engine. See the rewritePathForEngine
 * method above.
 */
module.exports.pathFromTemplates = function pathFromTemplates(/* path parts */) {
  var srcpath = this.rewritePathForEngine.apply(this, arguments);
 ...
 * the appropriate engine. See the rewritePathForEngine method above.
 */
 ...
 *
 *
 * XXX should replace the destpath extension with the correct extension
 * based on the engine registered for the type.
 */
module.exports.template = function template(srcpath, destpath, context) {
  ...
  return this.createFile(destpath, renderedTemplate)
};
