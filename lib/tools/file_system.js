 ...
 */
module.exports.engineAndTypeFromFilepath = function rewriteSourcePathForEngine(srcpath) {
  var re = /\.([A-Za-z]+)/g;
...
/**
 * Given a path of foo/bar.js and an engine for js that maps to
 * coffee, then rewrite the path to foo/bar.coffee
 */
module.exports.rewriteDestinationPathForEngine = function rewriteDestinationPathForEngine(/* path parts */) {
  ...
  var type = path.extname(destpath).replace('.', '');

  // if we have config engines then let's rewrite the
  // destpath extension for the appropriate engine. if destpath
  // is a directory and doesn't have a type, then nothing to do
  // here.
  if (type && config && config.engines) {
    var engine = config.engines[type];
    ...
    // if there's no engine defined just fall back
    // to using the destpath directly
    if (engine && engine !== type) {
      destpath = destpath.replace(type, engine);
    }
  }
...
/**
 * XXX the other problem is here. when we copy the template directory I copy all
 * files, not just the ones for the given engine. Need to account for that here.
 *
