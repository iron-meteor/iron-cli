...
module.exports.rewriteSourcePathForEngine = function rewriteSourcePathForEngine(/* path parts */) {
  ...
  if (!engineAndType.type)
    return srcpath;

  if (!config || !config.engines || !config.engines[engineAndType.type])
    return srcpath;
  ...
  if (engine === engineAndType.type)
    ret = srcpath;
  else
    ret = srcpath + '.' + config.engines[engineAndType.type];
