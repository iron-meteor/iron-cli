...
Command.create({
  ...
  generator.invoke(args.slice(1), opts);;
  ...
  CurrentConfig.withConfigFile(function() {
    var optionalGenerators;

    if (! _.isUndefined(this.CurrentConfig.get().generators[gName])) {
      optionalGenerators = this.CurrentConfig.get().generators[gName];
      for (g in optionalGenerators) {
        if (optionalGenerators[g]) {
