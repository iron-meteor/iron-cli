...
Command.create({
  ...
  var configPath = this.pathFromProject('config', appEnv).replace(/\\/g,'/');
  var envPath = path.join(configPath, 'env.sh').replace(/\\/g,'/');
  var settingsPath = path.join(configPath, 'settings.json').replace(/\\/g,'/');
