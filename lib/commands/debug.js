...
Command.create({
  ...
  var configPath = this.pathFromProject('config', appEnv).replace('\\','/');
  var envPath = path.join(configPath, 'env.sh').replace('\\','/');
  var settingsPath = path.join(configPath, 'settings.json').replace('\\','/');
