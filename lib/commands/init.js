...
// --skip-generator-comments=true|false
...
Command.create({
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    Iron.findGenerator('route').invoke(['home'], {cwd: projectDirectory});
    Iron.findGenerator('methods').invoke(['default'], {cwd: projectDirectory, where: 'both'});
    Iron.findGenerator('methods').invoke(['default'], {cwd: projectDirectory, where: 'server'});
