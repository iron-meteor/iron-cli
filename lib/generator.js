...
_.extend(Generator.prototype, {
  template: function (filepath, data) {
    try {
      ...
      var ejstmpl = ejs.render(tmpl, data);
      if (filepath !== 'both/router/route.js' && filepath !== 'both/router/routes.js' && filepath !== 'both/router/route.coffee' && filepath !== 'both/router/routes.coffee')
        return ejstmpl.trim();
      return ejstmpl;
    } catch (e) {
  ...
  writeFileWithTemplate: function (filepath, data) {
    return this.writeFile(filepath, this.template(filepath, data)); // template ends up becomming data for write file
  },

  injectRouteAtEndOfFile: function (filepath, content) {
    ...
    if (!allParts)
      throw new Error("injectRouteAtEndOfFile failed because the regular expression did not match anything in the file: " + re.source);
  ...
  injectContentIntoFile: function (filepath, content, begin, end) {
    ...
    if (!allParts)
      throw new Error("injectContentIntoFile failed because the regular expression did not match anything in the file: " + re.source);
